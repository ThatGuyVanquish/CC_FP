...
module Code_Generation : CODE_GENERATION= struct
  ...
  let collect_constants = 
    ...
    | ScmVarGet' (Var' (var, _)) -> [ScmSymbol var]
    ...
    | ScmVarSet' (var', expr') -> 
      (match var' with
        |Var' (var, _) -> [ScmSymbol var] @ (collector expr'))
    | ScmVarDef' (var', expr') -> 
      (match var' with
        |Var' (var, _) -> [ScmSymbol var] @ (collector expr'))
    | ScmBox' (Var'(var', _)) -> [ScmSymbol var']
    | ScmBoxGet' (Var'(var', _)) -> [ScmSymbol var']
  ...
  let make_lambda_opt_move_stack_up_loop = 
    make_make_label ".L_lambda_opt_move_stack_up_loop";;
  ...
  let make_lambda_opt_move_stack_up_end = 
    make_make_label ".L_lambda_opt_move_stack_up_end";;
  let make_lambda_opt_stack_setup_loop = 
    make_make_label ".L_lambda_opt_stack_setup_loop";;
  let make_lambda_stack_setup_end = 
    make_make_label ".L_lambda_opt_stack_setup_end";;
  let make_lambda_opt_opt_list_loop = 
    make_make_label ".L_lambda_opt_optional_list_loop";;
  let make_lambda_opt_opt_list_loop_end = 
    make_make_label ".L_lambda_opt_optional_list_end";;
  let make_lambda_opt_label_wrap_things_up = 
    make_make_label ".L_lambda_opt_wrap_things_up";;
  let make_tc_applic_recycle_frame_label = 
    make_make_label ".L_tc_recycle_frame_setup";;
  ...
  let code_gen exprs' =
    ...
    let rec run params env = function
      ...
      | ScmLambda' (params', Simple, body) ->
        ...
        and label_move_stack_up_loop = make_lambda_opt_move_stack_up_loop ()
        and label_move_stack_up_end = make_lambda_opt_move_stack_up_end ()
        and label_arity_more = make_lambda_opt_arity_more ()
        and label_stack_setup_loop = make_lambda_opt_stack_setup_loop ()
        and label_stack_setup_end = make_lambda_stack_setup_end ()
        and label_opt_list_loop = make_lambda_opt_opt_list_loop ()
        and label_opt_list_loop_end = make_lambda_opt_opt_list_loop_end ()
        and label_wrap_things_up = make_lambda_opt_label_wrap_things_up ()
        ...
        *)
        ...
        ^ (Printf.sprintf "%s:\n" label_opt_list_loop_end)
        (* 
          compare COUNT to size_of_params_tag
          if COUNT >= size_of_params_tag,  set PARAM(params_tag_size)
          to rdx then continue moving the stack up
          else move the stack down 1 slot then put nil there
        *)
        ^ (Printf.sprintf "\tcmp COUNT, %d\n" params_tag_size)
        ^ (Printf.sprintf "\tjg %s\n" label_arity_more)
        ^ "\tsub rsp, 8\n"
        ^ "\tmov rsi, 0\n"
        ^ (Printf.sprintf "\tmov r8, %d + 4\n" params_tag_size)
        ^ (Printf.sprintf "%s:\n" label_move_stack_up_loop)
        ^ "\tcmp rsi, r8\n"
        ^ (Printf.sprintf "\tje %s\n" label_move_stack_up_end)
        ^ "\tmov rdi, qword [rsp + 8 * (rsi + 1)]\n"
        ^ "\tmov qword [rsp + 8 * rsi], rdi\n"
        ^ "\tinc rsi\n"
        ^ (Printf.sprintf "\tjmp %s\n" label_move_stack_up_loop)
        ^ (Printf.sprintf "%s:\n" label_move_stack_up_end)
        ^ "\tmov qword [rsp + 8 * rsi], sob_nil\n"
        ^ "\tmov rbp, rsp\n"
        ^ (Printf.sprintf "\tmov COUNT, (%d + 1)\n" params_tag_size)
        ^ (Printf.sprintf "\tjmp %s\n" label_wrap_things_up)
        ^ (Printf.sprintf "%s:\n" label_arity_more)
        ^ (Printf.sprintf "\tmov PARAM(%d), rdx\n" (params_tag_size))
        ^ "\t\t\t; r8 will hold the amount of values left to push up\n"
        ^ "\t\t\t; initial value is old rbp + ret + env + count + List.length params' + opt\n"
        ...
        ^ "\tmov rdi,qword [rsi]\t; rdi holds the current value to be moved\n"
        ^ "\tmov qword [rsi + 8 * r9], rdi\n"
        ^ "\tdec r8\n"
        ^ "\tsub rsi, 8\n"
        ^ (Printf.sprintf "\tjmp %s\n" label_stack_setup_loop)
        ^ (Printf.sprintf "%s:\n" label_stack_setup_end)
      ...
      | ScmApplic' (proc, args, Tail_Call) ->
        let label_fix_stack = make_tc_applic_recycle_frame_label ()
        and label_stack_fixing_loop = make_tc_applic_recycle_frame_loop ()
        and label_stack_fixing_end = make_tc_applic_recycle_frame_done () in
        let args_code = List.fold_right (fun arg code -> 
          code
          ^(run params env arg)
          ^(Printf.sprintf "\tpush rax\n")
        ...
        ^ (Printf.sprintf "%s:\n" label_fix_stack)
        ^ "\t\t; NOTES:\n"
        ^ "\t\t; r8 = offset as in number of addresses\n"
        ^ "\t\t; r9 = amount of elements to be moved upwards\n"
        ^ "\t\t; rdx = pointer to element to be moved\n"

        ^ "\tmov r8, COUNT\t; put number of params of f (n) in r8\n"
        ^ "\tadd r8, 4\n"
        ^ "\tmov rbp, rsp\t; set rbp to act as g's rbp to use macros\n"
        ^ "\tmov r9, COUNT\n"
        ^ "\tadd r9, 4\n"
        ^ "\tlea rdx, [rbp + 8 * (r9 - 1)]\t; point rdx to the element to be moved\n"
        
        ^ (Printf.sprintf "%s:\n" label_stack_fixing_loop)
        ^ "\tcmp r9, 0\n"
        ^ (Printf.sprintf "\tje %s\n" label_stack_fixing_end)
        ^ "\tmov rsi, qword [rdx]\n"
        ^ "\tmov qword [rdx + 8 * r8], rsi\n"
        ^ "\tdec r9\n"
        ^ "\tsub rdx, 8\n"
        ^ (Printf.sprintf "\tjmp %s\n" label_stack_fixing_loop)
