...
L_code_ptr_bin_apply:
	push rbp
        mov rbp, rsp
	mov r8, PARAM(1) 	                ; r8 = list of parameter
	mov r9, 0			        ; size of list

.L_bin_apply_params_list_length_loop:
	cmp r8 , sob_nil
	je .L_bin_apply_params_list_length_end
	mov r8, SOB_PAIR_CDR(r8)
	inc r9
	jmp .L_bin_apply_params_list_length_loop

.L_bin_apply_params_list_length_end:
	mov r10, PARAM(1) 	                ; r10 = parameter list
	mov r11, PARAM(0) 	                ; r11 = function to apply

	mov COUNT, r9 		; update correct number of parameters on the stack
        mov r8, 2
        sub r8, r9              ; r8 holds the offset
	mov r9, 4 		; n env ret old_rbp (number of elements to shift)
	mov rdx, rsp
	cmp r8, 0

	je .L_bin_apply_shift_stack_end
	jl .L_bin_apply_shift_stack_down_setup

	; rdx will hold the address of the element to move
	; r8 will hold the direction (up or down, denoted by abs(8))
	; therefore the destination will be qword [rdx + 8 * r8]

.L_bin_apply_shift_stack_up_setup:
	mov rsi, 8				; direction is up
	add rdx, 8 * 3				; rdx points to COUNT								
	jmp .L_bin_apply_shift_stack_loop
		
.L_bin_apply_shift_stack_down_setup:	        
	mov rsi, -8			        ; direction is down

.L_bin_apply_shift_stack_loop:
	cmp r9, 0
	je .L_bin_apply_shift_stack_end
	mov rdi, qword [rdx]
	mov qword [rdx + 8 * r8], rdi
	dec r9 
	sub rdx, rsi
	jmp .L_bin_apply_shift_stack_loop

.L_bin_apply_shift_stack_end:
	lea rsp, [rsp + 8 * r8] 		; rsp = rsp + 8 * offset
	mov rbp, rsp 				; in order to use macros

	mov rax, SOB_CLOSURE_ENV(r11)           ; update env to be env of f
	mov ENV, rax

.L_bin_apply_insert_parameters_setup:
        mov r8, 0 				; r8 = parameter index
	mov r9, COUNT 			        ; we've already updated COUNT before stack shift
	
.L_bin_apply_insert_parameters_loop:
	; folding the parameter list onto the stack
	cmp r8, r9
	je .L_bin_apply_insert_parameters_end
        mov rsi, SOB_PAIR_CAR(r10) 
	mov PARAM(r8), rsi       ; r10 holds the list from earlier
	mov r10, SOB_PAIR_CDR(r10)
	inc r8
	jmp .L_bin_apply_insert_parameters_loop

.L_bin_apply_insert_parameters_end:
	pop rbp                                 ; because f will call ENTER
	jmp SOB_CLOSURE_CODE(r11) 		; jump to code of f
